...
Click **Save, then from the nodes window, click on the new node. That should show you the commands to connect it to the controller via JNLP once you have another node running on the same network. Save the commands to connect a Unix agent and proceed to the next step.
...
Pull up the commands from the end of step 1, and replace all instances of `localhost` with `jenkins`. This is because that is the container name of the jenkins controller defined in the docker compose file, and the agent will be able to find it on the docker network.
