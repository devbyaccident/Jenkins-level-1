...
First, you'll need to have a Git repository to make changes to. You're free to make one of your own, or clone the [Demo Repo from my account](https://github.com/devbyaccident/Demo_Repo).
...
Jenkins has it's own credential store for secrets that need to be used in build pipelines. To add it to Jenkins, go to the Jenkins home page, then click on **Manage Jenkins** > **Security** > **Credentials**
...
Now that the credentials are setup, it's time to use them. Open up your Hello World Freestyle job and open the configure page.
...
- Under **Build Triggers**, check the box next to **Poll SCM**, then enter `* * * * *` in the corresponding text box. This is a cron expression that will check the repository for a change every minute.

**Step 5: Poll Github from a Pipeline Job**
-------------------------------------------

Pipeline jobs are a little stricter in terms of what they'll allow configuration-wise. Instead of running the pipeline script from Jenkins as you've been doing, you're going to check it into SCM now.
...
Take the file named `Jenkinsfile` from this folder and copy it to the github repository you are polling in the Freestyle Job. 
